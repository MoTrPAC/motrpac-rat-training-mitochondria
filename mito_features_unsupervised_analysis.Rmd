...
for(tissue in tissues){
  ...
  is_mito = norm_data$feature_ID %in% mito_features
  norm_data_mito = norm_data[is_mito,norm_data_vials]
  norm_data_nonmito = norm_data[!is_mito,norm_data_vials]
  ...
  norm_data_mito$is_mito = T
  norm_data_nonmito$is_mito = F
  curr_data = rbind(
    norm_data_nonmito[,c("female,control","male,control","is_mito")],
    norm_data_mito[,c("female,control","male,control","is_mito")]
  )
...
ggplot(baseline_df,aes(x=tissue,y=male,fill=is_mito)) +
  geom_boxplot() +
  theme(axis.text.x = 
