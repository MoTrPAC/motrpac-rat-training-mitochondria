...
```{r}
rna_seq_dea = get(load("ihw_adjusted_mito_paper_dea_results.RData"))
dea_combined = get(load("ihw_adjusted_mito_paper_dea_prot_results.RData"))
...
for (ome in names(dea_combined)){
  for(tissue in unique(dea_combined[[ome]]$training_dea$tissue_abbr)){
    ...
    training_summary_df$not_robust = training_summary_df$before & !training_summary_df$after
...
tissues_for_adjustment_comparisons = 
  rownames(mt_group_kw)[rowSums(mt_group_kw<0.05,na.rm=T)>0]
...
par(mfrow = c(2,4))
for(j in 1:8){hist(abs(z_diffs[,j]),breaks=50,xlim=c(0,8),
                   main=colnames(zmat)[j])}
z_diff_features = apply(abs(z_diffs)>2,1,any)
...
binary_clustering_wrapper<-function(zmat,zthr=2,...){
  xmat = zmat > zthr
  mode(xmat) = "numeric"
  xmat[zmat < -zthr] = -1
  return(kmeans(xmat,...))
...
k_res = list()
for(k in 1:20){
  k_res[[k]] = kmeans(zmat[z_diff_features,],k,nstart = 10,iter.max = 500)
  # k_res[[k]] = binary_clustering_wrapper(zmat,3,centers=k,nstart = 10,iter.max = 500)
}
