...
for(tissue in names(mt_biomarkers)){
  ...
  new_max = curr_range[2] + 0.1 *(curr_range[2]-curr_range[1])
  p = ggboxplot(df,x="group",y="mt_reads",fill = curr_col,facet.by = "sex") + 
  stat_compare_means(method = "anova", label.y = new_max,size=5)+      # Add global p-value
  stat_compare_means(label = "p.signif", method = "t.test",
    ref.group = "control",symnum.args=symargs,size=6) + ylab("% mito reads (RNA-seq)") + xlab("") +
  ...
  if(!all(is.na(df$mtdna))){
    ...
    new_max = curr_range[2] + 0.1 *(curr_range[2]-curr_range[1])
    p = ggboxplot(df,x="group",y="mtdna",fill = curr_col,facet.by = "sex") + 
      stat_compare_means(method = "anova", label.y = new_max, size=5)+      # Add global p-value
      stat_compare_means(label = "p.signif", method = "t.test",
        ref.group = "control",symnum.args=symargs,size=6) + ylab("mtDNA levels") + xlab("") +
  ...
  if(!all(is.na(df$CL_72_8))){
    ...
    new_max = curr_range[2] + 0.1 *(curr_range[2]-curr_range[1])
     p = ggboxplot(df,x="group",y="CL_72_8",fill = curr_col,facet.by = "sex") + 
      stat_compare_means(method = "anova",label.y = new_max,size=5)+      # Add global p-value
      stat_compare_means(label = "p.signif", method = "t.test",
        ref.group = "control",symnum.args=symargs,size=6) + ylab("CL levels") + xlab("") +
