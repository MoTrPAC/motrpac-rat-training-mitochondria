...
for(tissue_name in unique(pheno$tissue)){
  ...
  for(s in c("female","male")){
    ...
    # perform KW test
    kw_res_v = rep(NA,5)
    names(kw_res_v) = c("mt_reads","mtdna","clPC1","clPC2","clPC3")
    ...
    effs = rep(NA,5)
    names(effs) = c("mt_reads","mtdna","clPC1","clPC2","clPC3")
    ps = rep(NA,5)
    names(ps) = names(effs)
  ...
  curr_cols = c("mt_reads","mtdna","clPC1","clPC2","clPC3")
...
ggcorrplot(mt_biomarker_correlations2,method = "circle")
ggcorrplot(abs(mt_biomarker_correlations2),method = "circle")
...
par(mar=c(8,8,8,8))
tissue_counts_8w = colSums(mt_biomarker_8wp<0.05,na.rm=T)
...
ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_female, y = clPC1_female, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) +
  xlim(c(-2.5,5)) + geom_abline(slope=1) + ggtitle("Female fold changes")
ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_male, y = clPC1_male, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) + 
  xlim(-7,6) + geom_abline(slope=1) + ggtitle("Male fold changes")


ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_female, y = clPC2_female, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) +
  xlim(c(-2.5,5)) + geom_abline(slope=1) + ggtitle("Female fold changes")
ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_male, y = clPC2_male, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) + 
  xlim(-7,6) + geom_abline(slope=1) + ggtitle("Male fold changes")

ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_female, y = clPC3_female, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) +
  xlim(c(-2.5,5)) + geom_abline(slope=1) + ggtitle("Female fold changes")
ggplot(mt_biomarker_8wfc,aes(
  x = mt_reads_male, y = clPC3_male, label=tissue
)) + geom_point() + geom_text(hjust=-0.1, vjust=0,size=4) + 
  xlim(-7,6) + geom_abline(slope=1) + ggtitle("Male fold changes")
