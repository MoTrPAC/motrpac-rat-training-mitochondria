...
for(ome in omes){
  ...
  for(tissue in tissues){
    ...
    x_bid = sapply(colnames(x),substring,1,5)
    additional_covs = additional_covs[x_bid,c("mt_reads","clPC1","clPC2")]
...
save(dea_results,file="mito_paper_dea_prot_results.RData")
```
...
save(dea_combined,file="ihw_adjusted_mito_paper_dea_prot_results.RData")
```


```{r}
load("ihw_adjusted_mito_paper_dea_prot_results.RData")
```
