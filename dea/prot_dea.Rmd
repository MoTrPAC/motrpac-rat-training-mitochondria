...
tissues = intersect(tissues,tissues_for_adjustment)
for(ome in omes){
  ...
  for(tissue in tissues){
    ...
    x_bid = sapply(colnames(x),substring,1,5)
    additional_covs = additional_covs[x_bid,]
    # subset the data frame to the selected markers 
    additional_covs = additional_covs[adjustment_features[[tissue]]]
    
    selected_mt_markers = colSums(is.na(additional_covs))==0
    mt_cov_names = colnames(additional_covs)[selected_mt_markers]
    # subset the data frame to the selected markers (after the NA filtering)
