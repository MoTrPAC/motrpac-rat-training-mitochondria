...
print("Overall test for overlap of selected genes in the studies:")
print(overall_test)
...
selection1 = all_shared_genes %in% stocks_d[!is.na(stocks_d$q) & stocks_d$q < 0.1,"symbol"]
selection2 = all_shared_genes %in% motrpac_d[!is.na(motrpac_d$selection_fdr) & 
                                           motrpac_d$selection_fdr<0.1,"gene_symbol"]
...
selected_sets[["ex_up_disease_down"]] = union(
  selected_sets$male_up_disease_down,
  selected_sets$female_up_disease_down
)
selected_sets[["ex_down_disease_up"]] = union(
  selected_sets$male_down_disease_up,
  selected_sets$female_down_disease_up
)

# Enrichment analysis using GO
res = gost(selected_sets[5:6],
    organism='rnorvegicus',
    significant=FALSE, # return all results
    correction_method='fdr', # it will not return unadjusted p-values. 
    sources=c('KEGG','REAC'),
    custom_bg = all_shared_genes,
    domain_scope='custom',
    evcodes=T)
