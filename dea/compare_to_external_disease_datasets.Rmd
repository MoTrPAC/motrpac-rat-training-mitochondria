...
add_overlap_tests_and_enrichments<-function(
  ...
  if(length(sign_table)>2){
    ...
    dataset_overlap_stats = rbind(dataset_overlap_stats,
      data.frame(dataset=datasetname,test = "sign_test_male",tissue=tissue,
      bg_size = sum(sign_table),overlap=0,p=NA))
  }
...
ggplot(df_overall_test,
  aes(x = name3,y = log10P,fill = tissue)) +
  geom_bar(stat = "identity",position = "dodge") + xlab("") + 
  geom_hline(yintercept = -log10(0.05)) + ylab("-log10 p-value") + 
  geom_text(aes(x = name3,y = log10P+0.3,label = Weight),
            vjust = 1.5, position = position_dodge(.9),
            col="black",size=4,angle=45) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1,size=11)) +
  theme(axis.text.y = element_text(size=8))


df$name4 = sapply(df$name3,function(x)paste(strsplit(x,split=",")[[1]][-3],collapse=","))
df$name4 = factor(df$name4,levels=unique(df$name4))
ggplot(df[!grepl("Overall",df$sex),],
  ...
  geom_hline(yintercept = -log10(0.05)) + ylab("-log10 p-value") + 
  geom_text(aes(x = name4,y = log10P+0.3,label = Weight),
            vjust = 1.5, position = position_dodge(.9),
            col="black",size=3,angle=30)
